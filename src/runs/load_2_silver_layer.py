...
def load_config() -> SilverConfig:
    bronze_root = os.environ.get("BRONZE_ROOT", os.path.join("artifacts", "bronze"))
    silver_root = os.environ.get("SILVER_ROOT", os.path.join("artifacts", "silver"))
    read_retries = int(os.environ.get("SILVER_READ_RETRIES", "2"))
    write_retries = int(os.environ.get("SILVER_WRITE_RETRIES", "2"))
    retry_backoff_s = float(os.environ.get("SILVER_RETRY_BACKOFF_S", "0.5"))
    log_json = os.environ.get("SILVER_LOG_JSON", "true").lower() in {"1", "true", "yes"}
    return SilverConfig(
...
def find_latest_bronze_run_id(bronze_root: str) -> str:
...
def standardize_missing_values(df: pd.DataFrame) -> pd.DataFrame:
    df = df.copy()
    missing_map = {"": pd.NA, "nan": pd.NA, "NaN": pd.NA, "null": pd.NA, "NULL": pd.NA}
...
def init_run_paths(config: SilverConfig, silver_run_id: str) -> SilverRunPaths:
    elt_dir = os.path.join(config.silver_root, silver_run_id)
    data_dir = os.path.join(elt_dir, "data")
    report_dir = os.path.join(elt_dir, "reports")
    ensure_dir(data_dir)
    ensure_dir(report_dir)
    harden_permissions(elt_dir, 0o750)
    harden_permissions(data_dir, 0o750)
    harden_permissions(report_dir, 0o750)
    log_file = os.path.join(data_dir, "run_log.txt")
    return SilverRunPaths(
        elt_dir=elt_dir,
        data_dir=data_dir,
...
def init_logger(log_file: str, log_json: bool) -> Callable[..., None]:
    def log(level: str, message: str, **fields: Any) -> None:
        ...
        line = json.dumps(payload, ensure_ascii=False) if log_json else f"{payload['ts']} | {level} | {message}"
...
def init_metadata(
    ...
    bronze_run_id: str,
    bronze_data_dir: str,
    bronze_metadata: Optional[Dict[str, Any]],
) -> Dict[str, Any]:
    metadata: Dict[str, Any] = {
        ...
        "source": {
            ...
            "bronze_artifact_path": os.path.dirname(bronze_data_dir),
...
def base_silver_cleaning(df: pd.DataFrame) -> pd.DataFrame:
    ...
    return standardize_missing_values(out)
...
def transform_LOC_A101(df: pd.DataFrame) -> pd.DataFrame:
    ...
    cols = {c.lower(): c for c in df.columns}
    if "cntry" in cols:
        df[cols["cntry"]] = df[cols["cntry"]].astype("string").str.strip().replace({"": pd.NA})
    return standardize_missing_values(df)
...
def transform_prd_info(df: pd.DataFrame) -> pd.DataFrame:
    ...
    if "prd_line" in df.columns:
        df["prd_line"] = df["prd_line"].astype("string").str.strip().replace({"": pd.NA})
    return standardize_missing_values(df)
...
def process_bronze_file(
    ...
    try:
        ...
        df = run_with_retries(
            ...
            retry_exceptions=(OSError, pd.errors.EmptyDataError, pd.errors.ParserError),
        )
        ...
        log("ERROR", "Traceback", file=filename, traceback=traceback.format_exc())
...
def main() -> int:
    ...
    config = load_config()
    bronze_run_id = sys.argv[1] if len(sys.argv) > 1 else find_latest_bronze_run_id(config.bronze_root)

    bronze_data_dir = os.path.join(config.bronze_root, bronze_run_id, "data")
    ...
    paths = init_run_paths(config, silver_run_id)
    log = init_logger(paths.log_file, config.log_json)

    log("INFO", "RUN_START", run_id=silver_run_id)
    log("INFO", "SOURCE", bronze_run_id=bronze_run_id)
    log(
        "INFO",
        "CONFIG",
    ...
    metadata["run"]["started_utc"] = iso_utc(start_dt)
    metadata["source"]["bronze_artifact_path"] = os.path.join(config.bronze_root, bronze_run_id)
    ...
    for filename in sorted(os.listdir(bronze_data_dir)):
        ...
        rec = process_bronze_file(filename, bronze_data_dir, paths.data_dir, config, log)
        results.append(rec)
        ...
        metadata["tables"][filename] = {
            ...
            "write_duration_s": rec["write_duration_s"],
            "input": {
                "path": os.path.join(bronze_data_dir, filename),
    ...
    report_html_path = os.path.join(paths.report_dir, "elt_report.html")
    report_ctx = {
        "run_id": silver_run_id,
        "bronze_run_id": bronze_run_id,
    ...
    if run_report_agent is not None:
        try:
            log("INFO", "Calling report agent", agent="load_2_report_agent.run_report_agent")
            run_report_agent(
                run_id=bronze_run_id,
                silver_run_id=silver_run_id,
            )
            log("INFO", "Report agent finished", agent="load_2_report_agent.run_report_agent")
        except Exception as e:
            # LLM/report failures must NOT break the ETL run
            log(
                "WARNING",
                "Report agent failed",
                agent="load_2_report_agent.run_report_agent",
    ...
    log(
        ...
        "RUN_END",
        run_id=silver_run_id,
        duration_s=metadata["run"]["duration_s"],
        success=ok,
        failed=failed,
    )
    log("INFO", "OUTPUT", output_dir=paths.elt_dir)
